;(function(){
    ...
    jplot.barplot = function (canvasId, dataset, /*optional*/ optionalArgs){
        ...
        if (beside){ //juxtaposed
            ...
            var sumHeights = [];
            var singleBarWidth = Math.round((1-2*offset)*canvas.width/barGroupCount*(1-offset));
            for (var i = 0; i < barGroupCount; i++){
                var temp = 0;
                for (var j = 0; j < dataSetCount; j++){
                    temp += realData[j][i];
            ...
            var maxHeight = maxElement(sumHeights, function(x){return x;});
            for (i = 0; i < barGroupCount; i++){
                var baseX = Math.round(2 * offset * canvas.width + i * (1 - 2*offset) * canvas.width/barGroupCount);
                var baseY = Math.round((1-offset) * canvas.height);
